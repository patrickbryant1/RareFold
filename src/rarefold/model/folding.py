...
class StructureModule(hk.Module):
  ...
  def loss(self, value, batch):
    ...
    ret['mod_sidechain_fape'] = sc_loss['mod_fape']
...
def sidechain_loss(batch, value, config):
  ...
  flat_positions_mod_mask = jnp.reshape(value['renamed_atom14_gt_exists']*jnp.expand_dims(mod_mask,axis=1), [-1])

  mod_fape = all_atom.frame_aligned_point_error(
      pred_frames=flat_pred_frames,
      target_frames=flat_gt_frames,
      frames_mask=flat_frames_mask,
      pred_positions=flat_pred_positions,
      target_positions=flat_gt_positions,
      positions_mask=flat_positions_mod_mask,
      l1_clamp_distance=config.sidechain.atom_clamp_distance,
      length_scale=config.sidechain.length_scale)
...
def find_structural_violations(
  ...
  atom14_atom_radius = batch['atom14_atom_exists'] * utils.batched_gather(jnp.array(atomtype_radius), batch['residx_atom14_to_atom37'])
...
def supervised_chi_loss(ret, batch, value, config):
  ...
  residue_type_one_hot = jax.nn.one_hot(
      batch['aatype'], residue_constants.restype_num , #Removed +1 here since this is already included in the mod residue_constants.py
      dtype=jnp.float32)[None]

  chi_pi_periodic = jnp.einsum('ijk, kl->ijl', residue_type_one_hot,
                               jnp.asarray([*residue_constants.chi_pi_periodic.values()]))
