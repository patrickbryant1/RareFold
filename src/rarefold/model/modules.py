"""Modules and code used in the core part of rarefold.

The structure generation code is in 'folding.py'.
"""
import functools
from rarefold.common import residue_constants
from rarefold.model import all_atom
from rarefold.model import common_modules
from rarefold.model import folding
from rarefold.model import layer_stack
from rarefold.model import lddt
from rarefold.model import mapping
from rarefold.model import prng
from rarefold.model import quat_affine
from rarefold.model import utils
import haiku as hk
import jax
...
class rarefoldIteration(hk.Module):
  """A single recycling iteration of rarefold architecture.
  ...
  def __init__(self, config, global_config, name='rarefold_iteration'):
  ...
  def __call__(self,
    ...
    if compute_loss:
      losses['sidechain_fape'] = ret['structure_module']['sidechain_fape']
      losses['mod_sidechain_fape'] = ret['structure_module']['mod_sidechain_fape']
      losses['lddt_ca'] = ret['predicted_lddt']['lddt_ca']
      losses['mod_lddt_ca'] = ret['predicted_lddt']['lddt_mod']
      losses['violations_extreme_ca_ca_distance'] = ret['structure_module']['metrics']['violations_extreme_ca_ca_distance']
      losses['violations_between_residue_bond'] = ret['structure_module']['metrics']['violations_between_residue_bond']
      losses['violations_between_residue_clash'] = ret['structure_module']['metrics']['violations_between_residue_clash']
      losses['violations_per_residue'] = ret['structure_module']['metrics']['violations_per_residue']
...
class RareFold(hk.Module):
  ...
  def __init__(self, config, name='rarefold'):
    super().__init__(name=name)
    self.config = config
  ...
  def __call__(
      ...
      ensemble_representations=False,
      return_representations=False):
    """Run the rarefold model.

    Arguments:
      batch: Dictionary with inputs to the rarefold model.
    ...
    Returns:
      When compute_loss is True:
        a tuple of loss and output of rarefoldIteration.
      When compute_loss is False:
        just output of rarefoldIteration.

      The output of rarefoldIteration is a nested dictionary containing
      predictions from the various heads.
    """

    impl = rarefoldIteration(self.config, self.global_config)
...
class PredictedLDDTHead(hk.Module):
  ...
  def loss(self, value, batch):
    ...
    #Get lDDT for the modified residues
    mod_mask = batch['mod_aa_index']
    mod_lddt_ca = jnp.sum(lddt_ca * mod_mask) / (jnp.sum(mod_mask) + 1e-8)
    #Normalise the lddt
    lddt_ca = jnp.sum(lddt_ca * mask_ca) / (jnp.sum(mask_ca) + 1e-8)


    output = {'loss': loss, 'lddt_ca': lddt_ca, 'lddt_mod': mod_lddt_ca}
    return output
