...
def get_chi_atom_indices():
  ...
  """
  chi_atom_indices = []
  for residue_name in residue_constants.residue_atoms:
...
def atom37_to_frames(
  ...
  num_atom_types = all_atom_positions.shape[-2]
  ...
  all_atom_positions = jnp.reshape(all_atom_positions, [-1, num_atom_types, 3])
  all_atom_mask = jnp.reshape(all_atom_mask, [-1, num_atom_types])

  # Create an array with the atom names.
  # shape (num_restypes, num_rigidgroups, 3_atoms): (21, 8, 3)
  restype_rigidgroup_base_atom_names = np.full([len(residue_constants.residue_atoms.keys())+1, 8, 3], '', dtype=object)
  ...
  # 4,5,6,7: 'chi1,2,3,4-group'
  #This is modified for RARE!!!

  for resname in residue_constants.residue_atoms:
    restype = residue_constants.resname_order[resname] #Get the index from the dict with the order
    for chi_idx in range(4):
      if residue_constants.chi_angles_mask[resname][chi_idx]:
  ...
  restype_rigidgroup_mask = np.zeros([len(residue_constants.residue_atoms.keys())+1, 8], dtype=np.float32)
  restype_rigidgroup_mask[:, 0] = 1
  restype_rigidgroup_mask[:, 3] = 1
  restype_rigidgroup_mask[:, 4:] = np.array([*residue_constants.chi_angles_mask.values()])
  ...
  restype_rigidgroup_is_ambiguous = np.zeros([len(residue_constants.chi_angles_mask), 8], dtype=np.float32)
  restype_rigidgroup_rots = np.tile(np.eye(3, dtype=np.float32), [len(residue_constants.chi_angles_mask), 8, 1, 1])

  #####Modified for RARE!!!#####
  for resname, _ in residue_constants.residue_atom_renaming_swaps.items():
    restype = np.argwhere(np.array([*residue_constants.chi_angles_mask.keys()])==resname)[0][0]
    chi_idx = int(sum(residue_constants.chi_angles_mask[resname]) - 1)
...
def between_residue_clash_loss(
  ...
  c_one_hot = jax.nn.one_hot(2, num_classes=25)
  n_one_hot = jax.nn.one_hot(0, num_classes=25)
  ...
  cys_sg_one_hot = jax.nn.one_hot(cys_sg_idx, num_classes=25)
